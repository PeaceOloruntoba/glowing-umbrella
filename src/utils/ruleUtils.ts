...
export const buildPreviewText = (rule: RuleUnit): string => {
  ...
  const childrenText = rule.children.map(buildPreviewText).join(` ${rule.relation?.toUpperCase()} `);
  return `(${childrenText})`;
};
...
export const evaluateRule = (rule: RuleUnit): boolean => {
  ...
  if (rule.children && rule.children.length) {
    const childResults = rule.children.map(evaluateRule);
    return rule.relation === 'and'
