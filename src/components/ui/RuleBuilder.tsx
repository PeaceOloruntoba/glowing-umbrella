...
const RuleBuilder: React.FC<RuleBuilderProps> = ({ rule, taskIndex, onUpdate, onRemove, level = 0 }) => {
  ...
  const removeChild = (childIndex: number) => {
    ...
    const updatedRule = { ...rule, children: children.length ? children : undefined };
    onUpdate(updatedRule);
    dispatch(updateRule({ taskIndex, rule: updatedRule }));
  };
  ...
  return (
    <Stack>
      <Group justify="apart">
        ...
        <Group>
          {rule.children?.map((_child, i) => (
            <ActionIcon key={i} size="sm" color="red" onClick={() => removeChild(i)}>
              <IconTrash size={16} />
            </ActionIcon>
          ))}
          <ActionIcon size="sm" color="red" onClick={onRemove}>
      ...
      <Group>
        <Button variant="light" size="xs" onClick={() => setShowAddCondition(true)}>
          + Condition
        </Button>
      ...
      <Divider />
      <Text size="sm" c="dimmed">Preview: {buildPreviewText(rule)}</Text>
      <Modal opened={showAddCondition} onClose={() => setShowAddCondition(false)} title="Add Condition">
        <Button fullWidth onClick={addCondition}>Add Value-Based Condition</Button>
      </Modal>
      <Modal opened={showAddGroup} onClose={() => setShowAddGroup(false)} title="Add Group">
        <Group>
          <Button variant="outline" onClick={() => addGroup('and')}>AND Group</Button>
          <Button variant="outline" onClick={() => addGroup('or')}>OR Group</Button>
        </Group>
      </Modal>
    </Stack>
  );
